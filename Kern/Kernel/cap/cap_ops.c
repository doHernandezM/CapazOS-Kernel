...
void cap_ops_selftest(cap_table_t *t)
{
    ...
    cap_handle_t h = 0;
    cap_status_t st = cap_create(t, CAP_TYPE_ENDPOINT, (CAP_R_READ | CAP_R_DUP | CAP_R_TRANSFER), (void *)0x1234, &h);
    expect(st == CAP_OK, "cap_create failed");

    cap_entry_t *e = cap_table_lookup(t, h, CAP_R_READ);
    expect(e != NULL, "cap_lookup failed");
    expect(e->type == CAP_TYPE_ENDPOINT, "type mismatch");
    ...
    // Dup/transfer smoke.
    cap_handle_t h1 = 0;
    st = cap_create(t, CAP_TYPE_VMO, (CAP_R_READ | CAP_R_DUP | CAP_R_TRANSFER), (void *)0xBEEF, &h1);
