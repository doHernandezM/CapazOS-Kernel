...
/* Find a node by absolute path and decode interrupts[0] into a GIC IRQ id. */
static bool dtb_find_node_gic_irq_by_path(const char *path, uint32_t *out_irq)
{
    if (!g_struct || !g_strings || !path || !out_irq) return false;
    if (path[0] != '/') return false;
    ...
    for (;;) {
        ...
        if (depth == target_depth && streq(pname, "interrupts")) {
            if (len < 12) return false;
            uint32_t itype = be32(data);
            uint32_t inum  = be32(data + 4);
            // uint32_t iflags = be32(data + 8);

            uint32_t irq = inum;
            // DTB interrupt specifier for GIC: <type num flags>
            // type: 0 = SPI, 1 = PPI
            if (itype == 0) irq = 32u + inum;
            else if (itype == 1) irq = 16u + inum;
...
/* Forward decl: used by DTB walkers before the helper is defined below. */
...
/* Walk DTB and find first PL011 node's reg + interrupts (QEMU virt model). */
static bool walk_struct_find_uart_irq(uint64_t *out_uart_phys, uint32_t *out_uart_irq)
{
    ...
    for (;;) {
        ...
        if (pname[0] == 'c' && pname[1] == 'o') { /* "compatible" */
            ...
            while (q < end && *q) {
                ...
                const char *u = needle;
                while (*t && *u && *t == *u) { t++; u++; }
                if (*u == '\0') { ctx->is_uart_candidate = true; break; }
                while (q < end && *q) q++;
                while (q < end && *q == '\0') q++;
            }
            continue;
        ...
        if (ctx->is_uart_candidate && streq(pname, "interrupts") && !found_irq) {
            if (len >= 12) {
                uint32_t itype = be32(data);
                uint32_t inum  = be32(data + 4);
                uint32_t irq = inum;
        ...
        if (ctx->is_uart_candidate && streq(pname, "reg") && !found_uart) {
    ...
    if (out_uart_phys) *out_uart_phys = uart_phys;
    if (out_uart_irq) *out_uart_irq = uart_irq;
    return found_uart && found_irq;
}
...
bool dtb_find_pl011_uart_irq(uint64_t *out_phys, uint32_t *out_irq)
{
    ...
    if (out_irq) *out_irq = 0;

    // Preferred: /chosen stdout-path -> node path -> reg + interrupts decode.
    uint64_t uart_phys = 0;
    char node_path[128] = {0};

    if (dtb_resolve_stdout_uart(&uart_phys, NULL, 0, node_path, (uint32_t)sizeof(node_path), NULL, 0) && uart_phys != 0) {
        uint32_t irq = 0;
        if (node_path[0] && dtb_find_node_gic_irq_by_path(node_path, &irq)) {
            if (out_phys) *out_phys = uart_phys;
            if (out_irq) *out_irq = irq;
            return true;
        }
        // If stdout-path didn't give interrupts, fall back to walking.
    }

    // Fallback: first compatible node with arm,pl011 (reg + interrupts).
    uint32_t irq2 = 0;
    uint64_t phys2 = 0;
    if (walk_struct_find_uart_irq(&phys2, &irq2)) {
        if (out_phys) *out_phys = phys2;
        if (out_irq) *out_irq = irq2;
        return true;
    }

    // Last-resort: provide reg only.
    if (dtb_find_pl011_uart(&uart_phys) && uart_phys != 0) {
        if (out_phys) *out_phys = uart_phys;
        return false;
    }
